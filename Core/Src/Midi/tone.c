...
//read required nums from the array
//duration will still have to be gotten as well

void play_tones () {
	uint32_t duration = tones[0].duration;
	uint32_t count = get_counter();
	int half = duration / 2;
	uint16_t power = 0;
	power += tones[0].power;
	if ((count % duration) < half){
		DAC_Set_Value(power *20);
	}
	else{
		DAC_Set_Value(0);
	}
